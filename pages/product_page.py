...
class ProductPage(BasePage):
    ...
    def should_be_alert_added_selected_item(self):
        assert self.browser.find_element(*ProductPageLocators.SUCCESS_ADDED_ITEM_ALERT).text == self.adding_item_params['name'], "ADDED ITEM NAME AND ITEM NAME IN ALERT AFTER ADDING DONT MATCH"

    def should_be_equal_item_cost_and_basket_cost_after_adding_item(self):
        assert self.browser.find_element(*ProductPageLocators.BASKET_TOTAL_COST_ALERT).text == self.adding_item_params['cost'], "ADDED ITEM COST AND BASKET TOTAL COST IN ALERT AFTER ADDING DONT MATCH"